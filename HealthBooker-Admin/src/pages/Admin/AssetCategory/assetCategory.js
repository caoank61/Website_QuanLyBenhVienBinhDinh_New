    ...
    const handleOkUser = async (values) => {
        ...
        try {
            return fieldtypesApi.addFieldType(values.type, values.status,).then(response => {
                if (response.message == "chuyên khoa bóng đã tồn tại") {
                    setLoading(false);
            ...
            return fieldtypesApi.updateFieldType(id, values.type, values.status).then(response => {
                if (response.message == "chuyên khoa bóng đã tồn tại") {
                    ...
                    return notification["error"]({
                        message: `Thông báo`,
                        description:
                            'Tên loại chuyên khoa đã tồn tại.',
                ...
                if (response === undefined) {
                    notification["error"]({
                        message: `Thông báo`,
                        description:
                            'Chỉnh sửa chuyên khoa thất bại',
                    });
                }
                else {
                    notification["success"]({
                        message: `Thông báo`,
                        description:
                            'Chỉnh sửa chuyên khoa thành công',
                    });
                    handleCategoryList();
    ...
    const handleDeleteCategory = async (id) => {
        ...
        try {
            await fieldtypesApi.deleteFieldType(id).then(response => {
                if (response.message === "Không thể xóa chuyên khoa này vì đã có sân liên kết đến nó.") {
                    notification["error"]({
                        message: `Thông báo`,
                        description:
                            "Không thể xóa chuyên khoa này vì đã có sân liên kết đến nó.",
                ...
                if (response === undefined) {
                    notification["error"]({
                        message: `Thông báo`,
                        description:
                            'Xóa chuyên khoa chuyên khoa thất bại',
                    ...
                    notification["success"]({
                        message: `Thông báo`,
                        description:
                            'Xóa chuyên khoa chuyên khoa thành công',
                ...
                <div>
                    <Row>
                        ...
                        <div
                            style={{ marginLeft: 10 }}>
                            <Popconfirm
                                title="Bạn có chắc chắn xóa chuyên khoa chuyên khoa này?"
    ...
    return (
        <div>
            <Spin spinning={loading}>
                            ...
                            >
                                <Row>
                                    ...
                                    <Col span="6">
                                        <Row justify="end">
                                            <Space>
                                                <Button onClick={showModal} icon={<PlusOutlined />} style={{ marginLeft: 10 }} >Tạo chuyên khoa</Button>
                                            </Space>
                                        </Row>
                                    </Col>
                                </Row>
                ...
                <Modal
                    title="Chỉnh sửa chuyên khoa chuyên khoa"
