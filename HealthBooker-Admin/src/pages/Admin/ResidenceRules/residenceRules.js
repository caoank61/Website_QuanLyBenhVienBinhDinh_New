...
const ResidenceRules = () => {
            ...
            return residenceRulesApi.createResidenceRule(categoryList).then(response => {
                if (response === undefined) {
                    notification["error"]({
                        message: `Thông báo`,
                        description:
                            'Tạo quy định hệ thống thất bại',
                    });
                }
                else {
                    notification["success"]({
                        message: `Thông báo`,
                        description:
                            'Tạo quy định hệ thống thành công',
                    });
            ...
            return residenceRulesApi.updateResidenceRule(categoryList, id).then(response => {
                if (response === undefined) {
                    notification["error"]({
                        message: `Thông báo`,
                        description:
                            'Chỉnh sửa quy định hệ thống thất bại',
                    });
                }
                else {
                    notification["success"]({
                        message: `Thông báo`,
                        description:
                            'Chỉnh sửa quy định hệ thống thành công',
                    });
                    handleCategoryList();
            ...
            await residenceRulesApi.deleteResidenceRule(id).then(response => {
                if (response === undefined) {
                    notification["error"]({
                        message: `Thông báo`,
                        description:
                            'Xóa quy định hệ thống thất bại',
                    ...
                    notification["success"]({
                        message: `Thông báo`,
                        description:
                            'Xóa quy định hệ thống thành công',
    ...
    const columns = [
        ...
        },
        {
            ...
            width: '60%',
            render: (text, record) => (
                <span>{record.content.length > 300 ? record.content.substring(0, 300) + "..." : record.content}</span>
            ),
        },
        {
            title: 'Hành động',
                ...
                <div>
                    <Row>
                        ...
                        <div
                            style={{ marginLeft: 10 }}>
                            <Popconfirm
                                title="Bạn có chắc chắn xóa quy định hệ thống này?"
                            ...
                            >
                                <Row>
                                    ...
                                    <Col span="6">
                                        <Row justify="end">
                                            <Space>
                                                <Button onClick={showModal} icon={<PlusOutlined />} style={{ marginLeft: 10 }} >Tạo quy định hệ thống</Button>
                                            </Space>
                                        </Row>
                                    </Col>
                                </Row>
 ...
 columns={columns} pagination={{ position: ['bottomCenter'] }} dataSource={category} />
                ...
                <Modal
                    title="Tạo quy định hệ thống mới"
                ...
                <Modal
                    title="Chỉnh sửa quy định hệ thống"
